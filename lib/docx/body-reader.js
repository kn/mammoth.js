...
function BodyReader(options) {
    ...
    function readFldChar(element) {
        ...
        if (type === "begin") {
            ...
            currentInstrText = [];
            // Handle checkbox form fields
            // NOTE: This logic is highly specific to documents that we see in the wild. It may not work for all documents.
            const ffData = element.children.find(child => child.name === "w:ffData");
            const checkbox = ffData && ffData.children.find(child => child.name === "w:checkBox");
            const checkedDefault = checkbox && checkbox.children.find(child => child.name === "w:default");
            if (checkedDefault && checkedDefault.attributes["w:val"]) {
                const checked = checkedDefault && checkedDefault.attributes["w:val"] === "1";
                return elementResult(new documents.Text(checked ? "☒" : "☐"));
            }
        } else if (type === "end") {
